 ...
 * Used by: Investigator Agent
 * Returns: Risk score (0-100, higher = more risky) + ERF (Employment Risk Factor)
 */
export function analyzeEmploymentRisk(employmentType: string, tenure: string): {
    riskScore: number;
    riskLevel: "Low" | "Medium" | "High" | "Critical";
    reason: string;
    erf: number; // Employment Risk Factor (0.40-1.00)
} {
    let riskScore = 0;
    let reason = "";
    let erf = 0.60; // Default ERF

    // Convert tenure string to months for ERF calculation
    const tenureMonths = tenure === "<6_months" ? 3
        : tenure === "6m-1yr" ? 9
            : tenure === "1-2yr" ? 18
                : tenure === "2-5yr" ? 42
                    : 72; // 5+yr

    // Base risk by employment type + Calculate ERF
    if (employmentType === "student") {
        riskScore += 70;
        reason = "Student employment is typically temporary and unverified.";
        erf = 0.40; // Students get heavy penalty
    ...
    return { riskScore, riskLevel, reason, erf };
}
