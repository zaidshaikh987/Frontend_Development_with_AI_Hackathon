...
const getNavigation = (lang: Language) => [
  {
    title: lang === "en" ? "YOUR APPLICATION" : lang === "hi" ? "आपका आवेदन" : "तुमचा अर्ज",
    items: [
      { name: getTranslation(lang, "dashboard"), href: "/dashboard", icon: LayoutDashboard },
      { name: getTranslation(lang, "applicationTimeline"), href: "/dashboard/timeline", icon: Calendar },
      { name: getTranslation(lang, "documentChecklist"), href: "/dashboard/documents", icon: FileCheck },
      { name: getTranslation(lang, "eligibilityReport"), href: "/dashboard/eligibility", icon: FileText },
    ]
  },
  {
    title: lang === "en" ? "LEARN" : lang === "hi" ? "सीखें" : "शिका",
    items: [
      { name: getTranslation(lang, "learningModules"), href: "/dashboard/learn", icon: BookOpen },
      { name: getTranslation(lang, "financialQuiz"), href: "/dashboard/quiz", icon: GraduationCap },
    ]
  },
  {
    title: lang === "en" ? "TOOLS" : lang === "hi" ? "उपकरण" : "साधने",
    items: [
      { name: getTranslation(lang, "loanComparison"), href: "/dashboard/loans", icon: TrendingUp },
      { name: getTranslation(lang, "creditPathOptimizer"), href: "/dashboard/optimizer", icon: Target },
      { name: getTranslation(lang, "multiGoalPlanner"), href: "/dashboard/multi-goal", icon: Goal },
    ]
  },
  {
    title: lang === "en" ? "SUPPORT" : lang === "hi" ? "सहायता" : "मदत",
    items: [
      { name: getTranslation(lang, "rejectionRecovery"), href: "/dashboard/rejection-recovery", icon: ShieldX },
      { name: getTranslation(lang, "peerInsights"), href: "/dashboard/peers", icon: Users },
      { name: lang === "en" ? "AI Assistant" : lang === "hi" ? "AI सहायक" : "AI सहाय्यक", href: "/dashboard/chat", icon: MessageSquare },
    ]
  },
]
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const [currentLang, setCurrentLang] = useState<string>("en") // Default for SSR
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 px-3 py-6 space-y-8 overflow-y-auto">
            {getNavigation(currentLang as Language).map((group, groupIdx) => (
              <div key={group.title}>
          ...
          {/* Bottom Actions with Language Selector */}
            ...
            <div className="px-2">
              ...
              <div className="flex gap-1">
                <button
                  ...
                  className={`flex-1 py-2 px-2 rounded-lg text-xs font-medium transition-all ${currentLang === "en"
                ...
                <button
                  ...
                  className={`flex-1 py-2 px-2 rounded-lg text-xs font-medium transition-all ${currentLang === "hi"
                    ? "bg-emerald-100 text-emerald-700 border-2 border-emerald-500"
                    : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                    }`}
                >
                ...
                <button
                  ...
                  className={`flex-1 py-2 px-2 rounded-lg text-xs font-medium transition-all ${currentLang === "mr"
                    ? "bg-emerald-100 text-emerald-700 border-2 border-emerald-500"
                    : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                    }`}
                >
